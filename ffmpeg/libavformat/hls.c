...
static int open_input(HLSContext *c, struct variant *var)
{
    ...
    if (seg->key_type == KEY_NONE) {
        ...
        if (strcmp(seg->key, var->key_url)) {
            URLContext *uc;
            for(int i=0;i<5;i++){
                if (ffurl_open(&uc, seg->key, AVIO_FLAG_READ,&var->parent->interrupt_callback, &opts) == 0) {
                    if (ffurl_read_complete(uc, var->key, sizeof(var->key))!= sizeof(var->key)) {
                        av_log(NULL, AV_LOG_ERROR, "HLS: Unable to read key file %s\n",seg->key);
                    }
                    ffurl_close(uc);
                    can_open = 1;
                    break;
                } else {
                    av_log(NULL, AV_LOG_ERROR, "HLS: try %d time, unable to open key file %s\n",i+1,seg->key);
                    ///wait a monent
                    av_usleep(100*1000);
                }
            }
...
start:
    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        if (var->needed && !var->pkt.data) {
            while (1) {
                ...
                if (ret < 0) {
					int feof = url_feof(&var->pb);
                    if (ret != AVERROR_EOF){//if (!url_feof(&var->pb) && ret != AVERROR_EOF)
						//av_log(NULL, AV_LOG_ERROR, "%s: read frame EOF error, ret=%d, feof=%d, var->pb.error=%d,\n",__FUNCTION__,ret,feof,var->pb.error);
                        return ret;
					}else{
						//av_log(NULL, AV_LOG_ERROR, "%s: read frame not EOF error, ret=%d, feof=%d, var->pb.error=%d,\n",__FUNCTION__,ret,feof,var->pb.error);
					}
                    reset_packet(&var->pkt);
                    break;
                } else {
				...
				//////for m3u8 seekable
				//av_log(NULL, AV_LOG_DEBUG, "%s break here\n\n",__FUNCTION__);
				break;
    ...
    if (minvariant >= 0) {
        *pkt = c->variants[minvariant]->pkt;
		...
		//av_log(NULL, AV_LOG_ERROR,"%s: read a packet, pkt->data=%0x,pkt->size=%d,pkt->pts=%lld,pkt->dts=%lld,pkt->buf=%0x,pkt->stream_index=%d\n",
		//			__FUNCTION__,pkt->data,pkt->size,pkt->pts,pkt->dts,pkt->buf,pkt->stream_index);
        return 0;
    }
	//av_log(NULL, AV_LOG_ERROR, "%s: out, return AVERROR_EOF\n",__FUNCTION__);
    return AVERROR_EOF;
}
    ...
    for (i = 0; i < c->n_variants; i++) {
        ...
        for (j = 0; j < var->n_segments; j++) {
            if (timestamp >= pos &&
                ...
                ret = 0;
				//av_log(NULL, AV_LOG_ERROR, "%s: timestamp=%lld,var[%d]->cur_seq_no=%d\n",__FUNCTION__,timestamp,i,var->cur_seq_no);
                break;
            }
