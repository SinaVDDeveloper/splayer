...
//int MediaPlayerSoft::packet_queue_put(PacketQueue *q, AVPacket *pkt);

int MediaPlayerSoft::packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
{
    MyAVPacketList *pkt1;
...
int MediaPlayerSoft::packet_queue_put_nullpacket(PacketQueue *q, int stream_index)
{
...
void MediaPlayerSoft::packet_queue_init(PacketQueue *q)
{
...
void MediaPlayerSoft::packet_queue_flush(PacketQueue *q)
{
...
/* flush half pack queue , add for audio video sync by lyh */
void MediaPlayerSoft::packet_queue_flush_half(PacketQueue *q)
{
    MyAVPacketList *pkt1;
	int i;
	
	int drop = q->nb_packets - 25;
	if(drop<=0){
		return;
	}
	av_log(NULL, AV_LOG_ERROR,"%s: videoq too long, flush to left 25 p!", __FUNCTION__);
...
void MediaPlayerSoft::packet_queue_destroy(PacketQueue *q)
{
    packet_queue_flush(q);
    LYH_DestroyMutex(q->mutex); 
    LYH_DestroyCond(q->cond);
}

void MediaPlayerSoft::packet_queue_abort(PacketQueue *q)
{
    LYH_LockMutex(q->mutex); 
...
void MediaPlayerSoft::packet_queue_start(PacketQueue *q)
{
    LYH_LockMutex(q->mutex);
    q->abort_request = 0;
...
//out: pkt ,serial
int MediaPlayerSoft::packet_queue_get(PacketQueue *q, AVPacket *pkt, int block, int *serial)
{
    MyAVPacketList *pkt1;
    int ret = 0;
...
void MediaPlayerSoft::blend_subrect(AVPicture *dst, const AVSubtitleRect *rect, int imgw, int imgh)
{
...
void MediaPlayerSoft::free_picture(VideoPicture *vp)
{
     if (vp->bmp) {
...
void MediaPlayerSoft::free_subpicture(SubPicture *sp)
{
...
void MediaPlayerSoft::video_image_display()
{
    VideoPicture *vp;
    SubPicture *sp;
    AVPicture pict;
...
///重绘最后一帧
void MediaPlayerSoft::video_redisplay()
{
    VideoPicture *vp;
    SubPicture *sp;
    AVPicture pict;
...
void MediaPlayerSoft::stream_close()
{
	...
	/////////////////////可能还没创建!
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: waiting read thread exit\n",__FUNCTION__);
    LYH_WaitThread(this->read_tid, NULL);
	this->read_tid = 0;
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: read thread end\n",__FUNCTION__);
	...
	if(this->screen){
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: close screen\n",__FUNCTION__);
	...
	//LYH_DestroyMutex(this->pause_lock);
	//this->pause_lock = NULL;

	
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: out\n",__FUNCTION__);
}

void MediaPlayerSoft::do_exit(int destroy_vs)
{
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);
...
//视频输出初始化
int MediaPlayerSoft::video_open(int force_set_video_mode, VideoPicture *vp)
{
...
void MediaPlayerSoft::video_display()
{
	int ret = 0;
    if (!this->screen){
        ret = video_open(0, NULL);
		if(-1==ret){
...
double MediaPlayerSoft::get_clock(Clock *c)
{
...
void MediaPlayerSoft::set_clock_at(Clock *c, double pts, int serial, double time)
{
    c->pts = pts;
    c->last_updated = time;
...
void MediaPlayerSoft::set_clock(Clock *c, double pts, int serial)
{
    double time = av_gettime() / 1000000.0;
    set_clock_at(c, pts, serial, time);
}

void MediaPlayerSoft::set_clock_speed(Clock *c, double speed)
{
    set_clock(c, get_clock(c), c->serial);
    c->speed = speed;
}

void MediaPlayerSoft::init_clock(Clock *c, int *queue_serial)
{
    c->speed = 1.0;
    c->paused = 0;
...
void MediaPlayerSoft::sync_clock_to_slave(Clock *c, Clock *slave)
{
...
int MediaPlayerSoft::get_master_sync_type() 
{
...
double MediaPlayerSoft::get_master_clock()
{
    double val;
...
*/
/* seek in the stream */
void MediaPlayerSoft::stream_seek(int64_t pos, int64_t rel, int seek_by_bytes)
{
    if (!this->seek_req) {
        this->seek_pos = pos;
        this->seek_rel = rel;
...
void MediaPlayerSoft::stream_toggle_pause(int unpause)
{
	//LYH_LockMutex(this->pause_lock); 
	...
	//LYH_UnlockMutex(this->pause_lock); 
	//av_log(NULL, AV_LOG_DEBUG,"%s: is->paused=%d",__FUNCTION__,is->paused);  
}

void MediaPlayerSoft::toggle_pause()
{
    stream_toggle_pause(0);
    this->step = 0;
}

void MediaPlayerSoft::step_to_next_frame()
{
...
double MediaPlayerSoft::compute_target_delay(double delay)
{
    ...
    //av_log(NULL, AV_LOG_ERROR, "%s: video: olddelay=%0.3f,delay=%0.3f A-V=%f\n",//av_dlog(NULL, "video: delay=%0.3f A-V=%f\n",
    //        __FUNCTION__,olddelay,delay, -diff);

    return delay;
}

void MediaPlayerSoft::pictq_next_picture() 
{
...
int MediaPlayerSoft::pictq_prev_picture() 
{
    VideoPicture *prevvp;
    int ret = 0;
...
void MediaPlayerSoft::update_video_pts(double pts, int64_t pos, int serial) 
{
	//av_log(NULL, AV_LOG_ERROR,"%s: into, pts=%lf,pos=%lld,serial=%d\n",__FUNCTION__,pts,pos,serial);
...
void MediaPlayerSoft::video_refresh(double *remaining_time)
{
    ...
    if (this->video_st) {
        ...
        if (this->force_refresh){
            redisplay = pictq_prev_picture();
        }
retry:
        if (this->pictq_size == 0) {
			//av_log(NULL, AV_LOG_ERROR,"%s: no picture to render\n",__FUNCTION__);
            LYH_LockMutex(this->pictq_mutex);
            if (this->frame_last_dropped_pts != AV_NOPTS_VALUE && this->frame_last_dropped_pts > this->frame_last_pts) {
                update_video_pts(this->frame_last_dropped_pts, this->frame_last_dropped_pos, this->frame_last_dropped_serial);
                this->frame_last_dropped_pts = AV_NOPTS_VALUE;
            }
            LYH_UnlockMutex(this->pictq_mutex);
            // nothing to do, no picture to display in the queue
            /*
            		is->no_picture_to_render++;
            		if(!is->eof && !is->seek_req && is->buffer_full && is->no_picture_to_render >= 10){
				video_redisplay(is);
				is->no_picture_to_render = 0;
			}
			if(is->no_picture_to_render>=100){
				is->no_picture_to_render = 0;
			}
			*/
            ...
            if (vp->serial != this->videoq.serial) { ///seek后不显示以前的图片
				//av_log(NULL, AV_LOG_ERROR,"%s: next pic, vp->serial=%d,this->videoq.serial=%d\n",__FUNCTION__,vp->serial,this->videoq.serial);
				pictq_next_picture();
                redisplay = 0;
                goto retry;
            }
            ...
            last_duration = vp->pts - this->frame_last_pts;
            if (!isnan(last_duration) && last_duration > 0 && last_duration < this->max_frame_duration) {
                /* if duration of the last frame was sane,神志正常的 update last_duration in MediaPlayer */
			...
			//av_log(NULL, AV_LOG_ERROR,"%s: vp->pts=%lf,frame_last_pts=%lf,frame_last_duration=%lf,delay=%lf,time=%lf,frame_timer=%lf,vp->serial=%d,this->videoq.serial=%d\n",
			//	__FUNCTION__,vp->pts,this->frame_last_pts,this->frame_last_duration,delay,time,this->frame_timer,vp->serial,this->videoq.serial);

			if (time < this->frame_timer + delay && !redisplay) {
                *remaining_time = FFMIN(this->frame_timer + delay - time, *remaining_time);
				//av_log(NULL, AV_LOG_ERROR,"%s: not show, remain time=%lf\n",__FUNCTION__,*remaining_time);
                return;
            }
			...
			///丢弃一当前帧?当前帧和下一帧时间已经过时
            if (this->pictq_size > 1) {
                VideoPicture *nextvp = &this->pictq[(this->pictq_rindex + 1) % VIDEO_PICTURE_QUEUE_SIZE];
                duration = nextvp->pts - vp->pts;
                if(!this->step && 
					(redisplay || this->framedrop>0 || (this->framedrop && get_master_sync_type() != AV_SYNC_VIDEO_MASTER)) &&
					time > this->frame_timer + duration ){
					//av_log(NULL, AV_LOG_ERROR,"%s: drop a late frame(time=%lf,is->frame_timer=%lf,duration=%lf,is->frame_drops_late_continue=%d)\n",
					//	__FUNCTION__,time,this->frame_timer,duration,this->frame_drops_late_continue);
					if (!redisplay){
                        this->frame_drops_late++;
						this->frame_drops_late_continue++;
					}
                    pictq_next_picture();
                    redisplay = 0;
                    goto retry;
                }else{ ///NEW ADD no else
					this->frame_drops_late_continue = 0;
				}
            }

            if (this->subtitle_st) {
                    while (this->subpq_size > 0) {
                        sp = &this->subpq[this->subpq_rindex];

                        if (this->subpq_size > 1)
                            sp2 = &this->subpq[(this->subpq_rindex + 1) % SUBPICTURE_QUEUE_SIZE];
                        else
                            sp2 = NULL;
                                ...
                                || (sp2 && this->vidclk.pts > (sp2->pts + ((float) sp2->sub.start_display_time / 1000))))
                        {
                            ...
                            /* update queue size and signal for next picture */
                            if (++this->subpq_rindex == SUBPICTURE_QUEUE_SIZE)
                                this->subpq_rindex = 0;
...
display:
            /* display picture */
            if (this->show_mode == SHOW_MODE_VIDEO) //if (!display_disable && is->show_mode == SHOW_MODE_VIDEO)
                video_display();

            pictq_next_picture();
			
			///如果是只放一帧则需要暂停,seekto发生时
			/*
	            if (is->step && !is->paused){
	                	stream_toggle_pause(is);
				if(is->send_seek_complete_notify){
        				play_notify(is,MEDIA_SEEK_COMPLETE, 0,0 , NULL);
...
int MediaPlayerSoft::alloc_picture()
{
    VideoPicture *vp;
    int64_t bufferdiff;
    ...
    if (!vp->bmp || vp->bmp->pitches[0] < vp->width || bufferdiff < (int64_t)vp->height * vp->bmp->pitches[0]) {
        /* allocates a buffer smaller than requested if the video overlay hardware is unable to support the requested size. */
...
int MediaPlayerSoft::queue_picture(AVFrame *src_frame, double pts, int64_t pos, int serial)
{
    VideoPicture *vp;
...
///#endif
    ...
    if (vp->bmp) {
        ...
        this->img_convert_ctx = sws_getCachedContext(this->img_convert_ctx,
        ...
        sws_scale(this->img_convert_ctx, src_frame->data, src_frame->linesize, 0, vp->height, pict.data, pict.linesize); ///sws_scale来进行图像缩放和格式转换，该函数可以使用各种不同算法来对图像进行处理
		*/
...
int MediaPlayerSoft::get_video_frame(AVFrame *frame, AVPacket *pkt, int *serial)
{
    int got_picture, ret;
	...
	///play MP3 bug fix
	if(this->abort_request){
		av_log(NULL, AV_LOG_DEBUG, "%s: this->abort_request\n",__FUNCTION__);
		return -1;
	}
	...
	if(this->videoq.nb_packets<=0 && !this->eof && this->audioq.nb_packets< this->MAX_FRAMES ){ //if(is->videoq.size<=0 && !is->eof && is->audioq.size< is->MAX_FRAMES ){ //if(is->videoq.size<=0){
		av_log(NULL, AV_LOG_DEBUG, "%s: is->videoq.nb_packets<=0\n",__FUNCTION__);
		if( this->buffer_full ){
			this->buffer_full = 0;
			this->notify(MEDIA_INFO, MEDIA_INFO_BUFFERING_START,0 , NULL);
			av_log(NULL, AV_LOG_DEBUG, "%s: set is->buffer_full = 0\n",__FUNCTION__);
		}
		return 0;
	}
	///获得一个pkt
	...
	///skip frame
	/*
		if( is->skip_frame >= 4 ) is->video_st->codec->skip_frame = (is->skip_auto==1)? AVDISCARD_BIDIR:AVDISCARD_NONREF;     //AVDISCARD_ALL:AVDISCARD_ALL;
		else if( is->skip_frame == 3 ) is->video_st->codec->skip_frame = (is->skip_auto==1)?AVDISCARD_NONKEY:AVDISCARD_NONKEY;
		else if( is->skip_frame == 2 ) is->video_st->codec->skip_frame = (is->skip_auto==1)?AVDISCARD_BIDIR:AVDISCARD_BIDIR;
		else if( is->skip_frame == 1 ) is->video_st->codec->skip_frame = (is->skip_auto==1)?AVDISCARD_NONREF:AVDISCARD_NONREF;
		else if( is->skip_frame == 0 ) is->video_st->codec->skip_frame = (is->skip_auto==1)?AVDISCARD_DEFAULT:AVDISCARD_DEFAULT;
		else if( is->skip_frame == -1 ) is->video_st->codec->skip_frame = (is->skip_auto==1)?AVDISCARD_NONE:AVDISCARD_NONE;
		else is->video_st->codec->skip_frame = (is->skip_auto==1)?AVDISCARD_DEFAULT:AVDISCARD_DEFAULT;
		///skip idct
		if( is->skip_idct >= 4 ) is->video_st->codec->skip_idct = (is->skip_auto==1)? AVDISCARD_BIDIR:AVDISCARD_NONREF;     //AVDISCARD_ALL:AVDISCARD_ALL;
		else if( is->skip_idct == 3 ) is->video_st->codec->skip_idct = (is->skip_auto== 1)?AVDISCARD_NONKEY:AVDISCARD_NONKEY;
		else if( is->skip_idct == 2 ) is->video_st->codec->skip_idct = (is->skip_auto==1)?AVDISCARD_BIDIR:AVDISCARD_BIDIR;
		else if( is->skip_idct == 1 ) is->video_st->codec->skip_idct = (is->skip_auto==1)?AVDISCARD_NONREF:AVDISCARD_NONREF;
		else if( is->skip_idct == 0 ) is->video_st->codec->skip_idct = (is->skip_auto==1)?AVDISCARD_DEFAULT:AVDISCARD_DEFAULT;
		else if( is->skip_idct == -1 ) is->video_st->codec->skip_idct = (is->skip_auto==1)?AVDISCARD_NONE:AVDISCARD_NONE;
		else is->video_st->codec->skip_idct = (is->skip_auto==1)?AVDISCARD_DEFAULT:AVDISCARD_DEFAULT;
		///skip loopfilter
		if( is->skip_loopfilter>= 4 ) is->video_st->codec->skip_loop_filter= (is->skip_auto==1)? AVDISCARD_BIDIR:AVDISCARD_NONREF;     //AVDISCARD_ALL:AVDISCARD_ALL;
		else if( is->skip_loopfilter == 3 ) is->video_st->codec->skip_loop_filter = (is->skip_auto==1)?AVDISCARD_NONKEY:AVDISCARD_NONKEY;
		else if( is->skip_loopfilter == 2 ) is->video_st->codec->skip_loop_filter = (is->skip_auto==1)?AVDISCARD_BIDIR:AVDISCARD_BIDIR;
		else if( is->skip_loopfilter == 1 ) is->video_st->codec->skip_loop_filter = (is->skip_auto==1)?AVDISCARD_NONREF:AVDISCARD_NONREF;
		else if( is->skip_loopfilter == 0 ) is->video_st->codec->skip_loop_filter = (is->skip_auto==1)?AVDISCARD_DEFAULT:AVDISCARD_DEFAULT;
		else if( is->skip_loopfilter == -1 ) is->video_st->codec->skip_loop_filter = (is->skip_auto==1)?AVDISCARD_NONE:AVDISCARD_NONE;
		else is->video_st->codec->skip_loop_filter = (is->skip_auto==1)?AVDISCARD_DEFAULT:AVDISCARD_DEFAULT;

		is->skip_auto= !is->skip_auto;
	*/
	if(!this->skip_auto_closed){
		if(this->frame_drop_step==0){
			if(this->video_st->codec->skip_frame != AVDISCARD_NONE){
				this->video_st->codec->skip_frame = AVDISCARD_NONE;
			}
			if(this->video_st->codec->skip_loop_filter != AVDISCARD_NONE){
				this->video_st->codec->skip_loop_filter = AVDISCARD_NONE;
				this->video_st->codec->skip_idct = AVDISCARD_NONE;
			}
		}else if(this->frame_drop_step==FRAME_DROP_STEP_1){
        ...
        if (this->framedrop>0 || (this->framedrop && get_master_sync_type() != AV_SYNC_VIDEO_MASTER)) {
            ...
            if (this->frame_last_pts != AV_NOPTS_VALUE && frame->pts != AV_NOPTS_VALUE) {
                ...
                if (!isnan(clockdiff) && fabs(clockdiff) < AV_NOSYNC_THRESHOLD &&
                    ...
                    //av_log(NULL, AV_LOG_DEBUG, "%s: drop a frame early,clockdiff=%lf,ptsdiff=%lf,dpts=%lf,is->videoq.nb_packets=%d,is->frame_drops_early_continue=%d\n",
					...
					if(!this->skip_auto_closed){
						...
						if(this->frame_drops_early_continue>FRAME_DROP_THRESHOLD){
							if( 0==this->frame_drop_step ){
								this->frame_drop_step = FRAME_DROP_STEP_1;
								av_log(NULL, AV_LOG_DEBUG,"%s: up to FRAME_DROP_STEP_1\n",__FUNCTION__);
								this->frame_drops_early_continue = 0;
							}else if( FRAME_DROP_STEP_1==this->frame_drop_step ){
								this->frame_drop_step = FRAME_DROP_STEP_2;
								av_log(NULL, AV_LOG_DEBUG,"%s: up to FRAME_DROP_STEP_2\n",__FUNCTION__);
								this->frame_drops_early_continue = 0;
							}
						}
					...
					if(!this->skip_auto_closed){
						//this->frame_drops_early_continue = 0;
						if(this->frame_drops_early_continue>0){
							this->frame_drops_early_continue--;
						}
						if(0>=this->frame_drops_early_continue){
							if(FRAME_DROP_STEP_2==this->frame_drop_step){
								if(!this->frame_drop_step_1_disable){
									if(0==this->last_frame_drop_step_1_time){
										this->last_frame_drop_step_1_time = (double)av_gettime();
									}else{
										double cur_frame_drop_step_time = (double)av_gettime();
										double diff_frame_drop_step_time = cur_frame_drop_step_time-this->last_frame_drop_step_1_time;
										diff_frame_drop_step_time = diff_frame_drop_step_time/ 1000000;
										av_log(NULL, AV_LOG_DEBUG,"%s: this->last_frame_drop_step_1_time=%lf,cur_frame_drop_step_time=%lf,diff_frame_drop_step_time=%lf\n",
											__FUNCTION__,this->last_frame_drop_step_1_time,cur_frame_drop_step_time,diff_frame_drop_step_time); 
										if(diff_frame_drop_step_time<=2){
											this->frame_drop_step_switch_1_count++;
										}else{
											this->frame_drop_step_switch_1_count = 0;
										}
										this->last_frame_drop_step_1_time = cur_frame_drop_step_time;
									}
									if(this->frame_drop_step_switch_1_count > 3){
										this->frame_drop_step_1_disable = 1;
									}
								}
								if(!this->frame_drop_step_1_disable){
									this->frame_drop_step = FRAME_DROP_STEP_1;
									av_log(NULL, AV_LOG_DEBUG,"%s: down to FRAME_DROP_STEP_1\n",__FUNCTION__); 
								}else{
									//av_log(NULL, AV_LOG_ERROR,"%s: disable down to FRAME_DROP_STEP_1\n",__FUNCTION__); 
								}
							}else if(FRAME_DROP_STEP_1==this->frame_drop_step){
								if(!this->frame_drop_step_0_disable){
									if(0==this->last_frame_drop_step_0_time){
										this->last_frame_drop_step_0_time = (double)av_gettime();
									}else{
										double cur_frame_drop_step_time = (double)av_gettime();
										double diff_frame_drop_step_time = cur_frame_drop_step_time-this->last_frame_drop_step_0_time;
										diff_frame_drop_step_time = diff_frame_drop_step_time/ 1000000;
										av_log(NULL, AV_LOG_DEBUG,"%s: this->last_frame_drop_step_0_time=%lf,cur_frame_drop_step_time=%lf,diff_frame_drop_step_time=%lf\n",
											__FUNCTION__,this->last_frame_drop_step_0_time,cur_frame_drop_step_time,diff_frame_drop_step_time); 
										if(diff_frame_drop_step_time<=4){
											this->frame_drop_step_switch_0_count++;
										}else{
											this->frame_drop_step_switch_0_count = 0;
										}
										this->last_frame_drop_step_0_time = cur_frame_drop_step_time;
									}
									if(this->frame_drop_step_switch_0_count > 3){
										this->frame_drop_step_0_disable = 1;
									}
								}
								if(!this->frame_drop_step_0_disable){
									this->frame_drop_step = 0;
									av_log(NULL, AV_LOG_WARNING,"%s: down to FRAME_DROP_STEP_0\n",__FUNCTION__); 
								}else{
									//av_log(NULL, AV_LOG_ERROR,"%s: disable down to FRAME_DROP_STEP_0\n",__FUNCTION__); 
								}
							}
						}
					}
...
///video解码线程
void* MediaPlayerSoft::video_thread(void *arg)
{
    AVPacket pkt = { 0 };
    MediaPlayerSoft *is = (MediaPlayerSoft *)arg;
    AVFrame *frame = av_frame_alloc();
    double pts;
    int ret;
    int serial = 0;

	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: video thread into\n",__FUNCTION__);
    ...
    for (;;) {
        ...
        if (!ret){
			av_log(NULL, AV_LOG_DEBUG, "%s: get_video_frame return 0\n",__FUNCTION__);
            continue;
        }

        pts = (frame->pts == AV_NOPTS_VALUE) ? NAN : frame->pts * av_q2d(is->video_st->time_base);
		//frame放进is里的队列去
		ret = is->queue_picture(frame, pts, av_frame_get_pkt_pos(frame), serial);
        av_frame_unref(frame);

        if (ret < 0){
			av_log(NULL, AV_LOG_ERROR, "%s: queue_picture return <0, quit video_thread\n",__FUNCTION__);
            goto the_end;
        }
    }
 the_end:
...
void* MediaPlayerSoft::subtitle_thread(void *arg)
{
    MediaPlayerSoft *is = (MediaPlayerSoft *)arg;
    SubPicture *sp;
    AVPacket pkt1, *pkt = &pkt1;
	...
	av_log(NULL, AV_LOG_ERROR, "%s: subtitle thread into\n",__FUNCTION__);
    ...
    for (;;) {
        ...
        if (is->subtitleq.abort_request){
			av_log(NULL, AV_LOG_ERROR, "%s: abort_request, subtitle thread exit\n",__FUNCTION__);
            return 0;
        }
...
int MediaPlayerSoft::synchronize_audio(int nb_samples)
{
...
int MediaPlayerSoft::audio_decode_frame()
{
    ...
    for (;;) {
        ...
        while (pkt_temp->stream_index != -1 || this->audio_buf_frames_pending) {
            ...
            if (this->audioq.serial != this->audio_pkt_temp_serial){
				//av_log(NULL, AV_LOG_ERROR,"%s: serial not same, audioq.serial=%d,this->audio_pkt_temp_serial=%d\n", __FUNCTION__,audioq.serial,this->audio_pkt_temp_serial);
                break;
            }
            ...
            if (!this->audio_buf_frames_pending) {
				...
				//av_log(NULL, AV_LOG_ERROR,"%s: decode pkt_temp->data=%0x,pkt_temp->size=%d,pkt_temp->pts=%lld,pkt_temp->dts=%lld,pkt_temp->buf=%0x,pkt_temp->stream_index=%d\n",
				//		__FUNCTION__,pkt_temp->data,pkt_temp->size,pkt_temp->pts,pkt_temp->dts,pkt_temp->buf,pkt_temp->stream_index);
		...
		if (this->audioq.nb_packets <= 0 && !this->eof){
			packet_queue_flush_half(&this->videoq);
			if( this->buffer_full ){
				av_log(NULL, AV_LOG_DEBUG,"%s: set is->buffer_full=0\n", __FUNCTION__);
				this->buffer_full = 0;
				this->notify(MEDIA_INFO, MEDIA_INFO_BUFFERING_START,0 , NULL);	
        ...
        if (pkt->data == this->flush_pkt.data) {
			//av_log(NULL, AV_LOG_ERROR,"%s: get a flush audio packet,pkt->data=%0x,this->flush_pkt.data=%0x\n", __FUNCTION__,pkt->data,this->flush_pkt.data);
		...
		//av_log(NULL, AV_LOG_ERROR,"%s: get audio packet, pkt->data=%0x,pkt->size=%d,pkt->pts=%lld,pkt->dts=%lld,pkt->buf=%0x,pkt->stream_index=%d,this->audio_pkt_temp_serial=%d\n",
		//						__FUNCTION__,pkt->data,pkt->size,pkt->pts,pkt->dts,pkt->buf,pkt->stream_index,this->audio_pkt_temp_serial);
        ...
        *pkt_temp = *pkt;
		...
		if(this->eof && pkt->data==NULL && !this->looping && !this->seek_req){
			...
			this->notify(MEDIA_PLAYBACK_COMPLETE, 0, 0 , NULL);
		}
    }
}
...
void MediaPlayerSoft::sdl_audio_callback(void *opaque, char *stream, int len)
{
    MediaPlayerSoft *is = (MediaPlayerSoft *)opaque;
    int audio_size, len1;
    int bytes_per_sec;
	...
	//av_log(NULL, AV_LOG_ERROR,"%s out,is->audio_clock=%lf,tmp_pts=%lf,is->audio_write_buf_size=%d,is->audio_clock_serial=%d\n", __FUNCTION__, is->audio_clock, tmp_pts,is->audio_write_buf_size,is->audio_clock_serial );
}
///返回缓冲区大小?

int MediaPlayerSoft::audio_open( int64_t wanted_channel_layout, int wanted_nb_channels, int wanted_sample_rate, struct AudioParams *audio_hw_params)
{
    ...
    av_log(NULL, RELEASE_DEBUG_LOG_LEVEL,"%s into,wanted_channel_layout=%lld,wanted_nb_channels=%d,wanted_sample_rate=%d\n", 
...
///move from cmd.cc
int MediaPlayerSoft::check_stream_specifier(AVFormatContext *s, AVStream *st, const char *spec)
{
...
AVDictionary *MediaPlayerSoft::filter_codec_opts(AVDictionary *opts, enum AVCodecID codec_id,
...
int MediaPlayerSoft::stream_component_open(int stream_index)
{
	...
	if (!codec) {                
		av_log(NULL, AV_LOG_ERROR,"No codec could be found with id[%d] name[%s]\n", avctx->codec_id,avctx->codec_name);
        return -1;
    }
    ...
    opts = filter_codec_opts(this->codec_opts, avctx->codec_id, ic, ic->streams[stream_index], codec);
    ...
    if (avcodec_open2(avctx, codec, &opts) < 0){ 
		av_log(NULL, AV_LOG_ERROR, "%s: avcodec_open2 open codec fail\n", __FUNCTION__);
		return -1;
    }
	
	//////////must remove
	//av_log(NULL, AV_LOG_INFO, "%s: extradata dump\n",__FUNCTION__);
	//for(int j=0;j<avctx->extradata_size;j++){
	//	av_log(NULL, AV_LOG_INFO, "%s: extradata[%d]=%d\n",__FUNCTION__,j,avctx->extradata[j]); 
	//}
	//av_log(NULL, AV_LOG_INFO, "%s: extradata dump end\n",__FUNCTION__);
	//////////
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        break;
    case AVMEDIA_TYPE_VIDEO:
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: opening video stream\n",__FUNCTION__);
        ...
        break;
    case AVMEDIA_TYPE_SUBTITLE:
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: opening subtitle stream\n",__FUNCTION__);
...
void MediaPlayerSoft::stream_component_close(int stream_index)
{
    AVFormatContext *ic = this->ic;
    AVCodecContext *avctx;
    ...
    switch (avctx->codec_type) {
    case AVMEDIA_TYPE_AUDIO:
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: closing audio\n",__FUNCTION__);
		...
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: audio closed \n",__FUNCTION__);
        break;
    case AVMEDIA_TYPE_VIDEO:
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: closing video\n",__FUNCTION__);
		...
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: video closed \n",__FUNCTION__);
        break;
    case AVMEDIA_TYPE_SUBTITLE:
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: closing subtitle\n",__FUNCTION__);
		...
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: subtitle closed\n",__FUNCTION__);
        break;
    default:
        break;
    }
	...
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: out,stream_index=%d end\n",__FUNCTION__,stream_index);
}

int MediaPlayerSoft::decode_interrupt_cb(void *ctx)
{
	av_log(NULL, AV_LOG_INFO, "%s: into\n",__FUNCTION__);

    MediaPlayerSoft *is = (MediaPlayerSoft *)ctx;
    return is->abort_request;
}

int MediaPlayerSoft::is_realtime(AVFormatContext *s)
{
	if(!s){
		return 0;
	}
...
int MediaPlayerSoft::is_seekable(AVFormatContext *s)
{
	if(NULL!=s && NULL!=s->pb){
...
///move from cmd.cc
AVDictionary **MediaPlayerSoft::setup_find_stream_info_opts(AVFormatContext *s,
...
void* MediaPlayerSoft::read_thread(void *arg)
{
	//int eof = 0;
	int err, i, ret;
	AVPacket pkt1, *pkt = &pkt1;
	...
	if(!is->buffer_full){
		av_log(NULL, AV_LOG_DEBUG, "%s: first notify buffering start\n",__FUNCTION__);
		is->notify(MEDIA_INFO, MEDIA_INFO_BUFFERING_START,0 , NULL);
	}
 
    for (;;) {
        if (is->abort_request){
			av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: is->abort_request true\n",__FUNCTION__);
            break;
        }
        ...
        if (is->seek_req) {
			if(is->is_seekable(is->ic)){
	            ...
	            if (ret < 0) {
	                av_log(NULL, AV_LOG_ERROR,"%s: error while seeking(%s)\n", __FUNCTION__,is->ic->filename);
	            } else {
	            	/*if(is->send_seek_complete_notify){
	            				play_notify(is,MEDIA_SEEK_COMPLETE, 0,0 , NULL);
						is->send_seek_complete_notify = 0;
	            			}*/
	            	av_log(NULL, AV_LOG_DEBUG,"%s: set buffer_full=0 at seek request\n", __FUNCTION__);
	            	is->buffer_full = 0;
				...
				//if (is->paused){ //seek之后会主动开始播放
	            //    is->stream_toggle_pause(); //step_to_next_frame(is);
				...
				//av_log(NULL, AV_LOG_ERROR,"%s: hls living not support seeking\n",__FUNCTION__);
				is->seek_req = 0;
				//if (is->paused){
	                is->stream_toggle_pause(1); //step_to_next_frame(is);
	                if( is->state == SPALYER_STATE_PAUSED ){
		...
		if ( ( is->eof ||
			  (   (is->audioq.nb_packets > is->MAX_FRAMES || is->audio_stream < 0 || is->audioq.abort_request) && 
			       ...
			       //(is->subtitleq.nb_packets > is->MAX_FRAMES || is->subtitle_stream < 0 || is->subtitleq.abort_request) ) ) ) { //开始时候没字幕BUG : nb_packets=0 subtitle_stream>=0 subtitleq.abort_request=0
			// wait 10 ms
            if(!is->buffer_full){
				if(is->eof){
				...
				//av_log(NULL, AV_LOG_DEBUG, "%s: set buffer full,is->audioq.nb_packets=%d,is->videoq.nb_packets=%d\n",__FUNCTION__,is->audioq.nb_packets, is->videoq.nb_packets);
            }
			if(!is->eof){
		...
		if(!is->eof && !is->buffer_full){
			//av_log(NULL, AV_LOG_DEBUG, "%s: buffering... is->audioq.size=%d,is->videoq.size=%d,is->subtitleq.size=%d,is->audioq.nb_packets=%d,is->videoq.nb_packets=%d,is->subtitleq.nb_packets=%d,is->audioq.abort_request=%d,is->videoq.abort_request=%d,is->subpq.abort_request=%d,is->eof=%d\n", 
			//				__FUNCTION__,is->audioq.size,is->videoq.size,is->subtitleq.size,is->audioq.nb_packets,is->videoq.nb_packets,is->subtitleq.nb_packets,is->audioq.abort_request,is->videoq.abort_request,is->subtitleq.abort_request,is->eof);
        ...
        if (ret < 0) {
			//if(is->ic->pb){
			//	av_log(NULL, AV_LOG_DEBUG, "%s: is->ic->pb->error=%d\n",__FUNCTION__,is->ic->pb->error);
			//}
			//if(ret == AVERROR_EOF){
            ...
            if (ret == AVERROR_EOF || url_feof(is->ic->pb)){
				if(ret == AVERROR_EOF){
					...
					//if(strncmp(is->ic->iformat->name,"hls",3)){
						...
						*/
						if(is->ic->pb && is->ic->pb->error<0){ //无网或者读错误时还能继续
				...
				//av_log(NULL, AV_LOG_ERROR, "%s: reach file end\n",__FUNCTION__);
                ///is->eof = 1;
            }else{
            ...
            //if (is->ic->pb && is->ic->pb->error){
				///测试无网或者读错误时还能继续
				///play_notify(is,MEDIA_ERROR, MEDIA_ERROR_SERVER_DIED,MEDIA_ERROR_IO, NULL);
...
int MediaPlayerSoft::videostate_init(AVInputFormat *iformat)
{
	...
	//config
	//this->fast = 0;  //default is 0
	//this->lowres = 0;  //default is 0
	if(0==this->framedrop)
		this->framedrop = -1;
    ...
    ////init_clock(&this->extclk, &this->extclk.serial);
	...
	//this->pause_lock = LYH_CreateMutex();;
...
int MediaPlayerSoft::do_stream_open()
{
	...
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: avformat_open_input begin,is->filename=%s\n",__FUNCTION__,this->filename);
	err = avformat_open_input(&ic, this->filename, this->iformat, &this->format_opts);
	if (err < 0) {
		av_log(NULL, AV_LOG_ERROR, "%s: avformat_open_input fail,err=%d\n",__FUNCTION__,err);
		ret = -1;
		goto fail;
	}
   	if ((t = av_dict_get(this->format_opts, "", NULL, AV_DICT_IGNORE_SUFFIX))) {
	   av_log(NULL, AV_LOG_ERROR, "Option %s not found.\n", t->key);
	...
	this->duration = this->ic->duration;
   	...
   	opts = setup_find_stream_info_opts(ic, this->codec_opts);
	...
	//av_log(NULL, AV_LOG_ERROR, "%s: this->max_frame_duration=%lf\n",__FUNCTION__,this->max_frame_duration);
	   ...
	   st_index[AVMEDIA_TYPE_VIDEO] =
	    ...
	    if(st_index[AVMEDIA_TYPE_VIDEO] >=0 ){
	   		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: av_find_best_stream find video stream\n",__FUNCTION__);
	    }
   	}
	   ...
	   if(st_index[AVMEDIA_TYPE_AUDIO] >=0 ){
	   		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: av_find_best_stream find audio stream\n",__FUNCTION__);
	   }
   	}
	   ...
	   if(st_index[AVMEDIA_TYPE_SUBTITLE]>=0){
	       av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: av_find_best_stream find subtitle stream\n",__FUNCTION__);
	   }	   
   	}
   	if (0) { //if (show_status) {
...
fail:
    ...
    if (ret != 0) {
        ...
        event.type = LYH_QUIT_EVENT;//FF_QUIT_EVENT;
        event.data = is; ///event.user.data1 = is;
        LYH_PushEvent(&event);
    }
    */
	av_log(NULL, AV_LOG_ERROR, "%s: open stream fail\n",__FUNCTION__);

	return ret;
}

int MediaPlayerSoft::stream_start()
{
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);
...
int MediaPlayerSoft::stream_open(AVInputFormat *iformat)
{
    //MediaPlayer *is;
	int ret;
...
void MediaPlayerSoft::refresh_loop_wait_event(LYH_event *event)
{
    double remaining_time = 0.0;
	int ret;
    /*
    while (!LYH_PeepEvents(this->hardware_context,event, LYH_ALLEVENTS)) {
        if (remaining_time > 0.0){
            av_usleep((int64_t)(remaining_time * 1000000.0));
        }
        remaining_time = REFRESH_RATE;
        if ( (this->show_mode != SHOW_MODE_NONE) && this->buffer_full && (!this->paused || this->force_refresh)){ //if (is->show_mode != SHOW_MODE_NONE && (!is->paused || is->force_refresh)){
     ...
     do{
		...
		if(1==ret){
			if(event->type==LYH_SEEK_EVENT && this->seeking){
				av_log(NULL, AV_LOG_WARNING, "%s: push seeking event when seeking\n",__FUNCTION__);
				LYH_PushEvent(this->hardware_context,event); 
			}else{
				break;
			}
...
int MediaPlayerSoft::seek_to(int msec)
{
  	int64_t ts;
...
/* handle an event sent by GUI */
void MediaPlayerSoft::event_loop()
{
    LYH_event event;
    double incr, pos;
	int msec;
    ...
    for (;;) {
        ...
        switch (event.type) {
        case LYH_QUIT_EVENT: 
        	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: recv a quit event\n",__FUNCTION__);
            do_exit(1);
			return ;
		case LYH_RESET_EVENT: 
        	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: recv a reset event\n",__FUNCTION__);
            do_exit(0);
			return ;
...
void* MediaPlayerSoft::event_thread(void *arg)
{
	MediaPlayerSoft *is = (MediaPlayerSoft *)arg;
...
int MediaPlayerSoft::lockmgr(void **mtx, enum AVLockOp op)
{
   switch(op) {
      case AV_LOCK_CREATE:
          *mtx = LYH_CreateMutex();
...
void* MediaPlayerSoft::stream_open_thread(void *arg)
{
	int ret;
	MediaPlayerSoft *is = (MediaPlayerSoft *)arg;
	...
	if (av_lockmgr_register(lockmgr)) {
		av_log(NULL, AV_LOG_FATAL, "%s: Could not initialize lock manager!\n",__FUNCTION__);
		if(is->state==SPALYER_STATE_PREPARING){
			goto fail;
		}
		is->stream_open_thread_id = 0;
		return NULL; //return SPLAYER_ERROR_INTERNAL;
	}
	...
	if (ret<0) {
		if(is->state==SPALYER_STATE_PREPARING){
			av_log(NULL, AV_LOG_ERROR, "%s:failed to open stream, in SPALYER_STATE_PREPARING!\n",__FUNCTION__);
			goto fail;
		}
		is->stream_open_thread_id = 0; //-1;
		av_log(NULL, AV_LOG_ERROR, "%s: failed to open stream, exit thread!\n",__FUNCTION__);
		return NULL; //return SPLAYER_ERROR_NETWORK;
	}
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: stream_open end\n",__FUNCTION__);

	///event thread, handle event and show video
...
void MediaPlayerSoft::player_log_callback_help(void *ptr, int level, const char *fmt, va_list vl)
{
...
MediaPlayerSoft::MediaPlayerSoft()
{
	this->read_tid = 0;
	this->video_tid = 0;
	this->iformat = NULL;
	...
	this->frame_drops_late_continue = 0;
	this->frame_drop_step = 0;
	this->last_frame_drop_step_0_time = 0;
	this->frame_drop_step_switch_0_count = 0;
	this->frame_drop_step_0_disable = 0;
	this->last_frame_drop_step_1_time = 0;
	this->frame_drop_step_switch_1_count = 0;
	this->frame_drop_step_1_disable = 0;
	//this->pause_lock = NULL;
	//hrad decoder
...
int MediaPlayerSoft::using_hard_decode(void)
{
	return 0;
}


int MediaPlayerSoft::setup(JavaVM* jvm, jobject thiz, jobject weak_this)
{
	av_log(NULL,RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);
	...
	if (listener == NULL) {
		av_log(NULL,AV_LOG_ERROR, "%s: new JNIMediaPlayerListener fail\n",__FUNCTION__);
...
int MediaPlayerSoft::set_surfaceview(JavaVM* jvm,jobject jsurfaceview)
{
	av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);

	if(!jsurfaceview || !jvm){
		av_log(NULL, AV_LOG_ERROR, "%s: jvm or jsurfaceview NULL\n",__FUNCTION__);
	...
	JNIAndroidOpenGl2RendererListener* listener = new JNIAndroidOpenGl2RendererListener(jvm, jsurfaceview);
	if (listener == NULL) {
		av_log(NULL, AV_LOG_ERROR, "%s: new JNIAndroidOpenGl2RendererListener() outof memory\n",__FUNCTION__);
        return SPLAYER_ERROR_NO_MEMORY;
    }
    this->opengl2_render_listener = listener;
...
int MediaPlayerSoft::set_surface(JavaVM* jvm,void* surface)
{
	av_log(NULL, AV_LOG_ERROR, "%s: soft decoder unsupport surface\n",__FUNCTION__);
	return SPLAYER_ERROR_SPALYER_STATE;
}


int MediaPlayerSoft::set_datasource(const char* path)
{
	av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);
...
int MediaPlayerSoft::set_video_scaling_mode(int mode)
{
...
int MediaPlayerSoft::set_looping(int loop)
{
	av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);
...
int MediaPlayerSoft::prepareAsync()
{
	int ret;
	...
	if(this->state == SPALYER_STATE_PREPARING ){
		av_log(NULL, AV_LOG_WARNING, "%s: already preparing\n",__FUNCTION__);
		return SPLAYER_SUCCESS;
	}

	splayer_state last_state = this->state;
	this->state = SPALYER_STATE_PREPARING;
	...
	if(0==this->event_thread_id){
		...
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s create event thread success(%lu)\n",__FUNCTION__,this->event_thread_id);
	}
	...
	if(0==this->stream_open_thread_id){
		this->state = last_state;
		av_log(NULL, AV_LOG_FATAL, "%s create stream open thread fail\n",__FUNCTION__);
		return SPLAYER_ERROR_INTERNAL;
	}else{
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s create stream open thread success(%lu)\n",__FUNCTION__,this->stream_open_thread_id);
	}
...
int MediaPlayerSoft::release()
{
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: into\n",__FUNCTION__);

	if( this->state == SPALYER_STATE_END){
		av_log(NULL, AV_LOG_DEBUG, "%s: already release\n",__FUNCTION__);
		return SPLAYER_SUCCESS;
	}
	...
	//if(this->stream_open_thread_id != 0){
		av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: wait stream open thread exit\n",__FUNCTION__);
		LYH_WaitThread(this->stream_open_thread_id,NULL);
	//}

	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: push quit event\n",__FUNCTION__);
	...
	av_log(NULL, RELEASE_DEBUG_LOG_LEVEL, "%s: waiting event thread exit\n",__FUNCTION__);
...
int MediaPlayerSoft::seekto(int msec)
{
	//av_log(NULL, AV_LOG_WARNING, "%s: into, msec=%ld,is->seeking=%d\n",__FUNCTION__,msec,this->seeking);
	...
	if(this->state==SPALYER_STATE_PREPARED){
		if(0 == this->read_tid){
			...
			if (0 == this->read_tid) {
				...
				av_log(NULL, AV_LOG_ERROR, "%s: create read thread ssuccess(%lu)\n",__FUNCTION__,this->read_tid);
			}
		}
	}
	//if(1==this->seeking){
	...
	//av_log(NULL, AV_LOG_WARNING, "%s: out\n",__FUNCTION__);
	
	return SPLAYER_SUCCESS;
}

int MediaPlayerSoft::is_playing()
{
	av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);
...
int MediaPlayerSoft::get_video_width()
{
	return this->frame_width;
}

int MediaPlayerSoft::get_video_height()
{
	return this->frame_height;
}

float MediaPlayerSoft::get_video_aspect_ratio()
{
...
int MediaPlayerSoft::get_current_position()
{
	int msec = 0;
	if( is_realtime(this->ic) ){
	...
	if(msec<0){
		av_log(NULL, AV_LOG_ERROR, "%s: this->audio_clock not set yet\n",__FUNCTION__);
		msec = 0;
	}
...
int MediaPlayerSoft::get_duration()
{
	int msec = this->duration/1000 ;
	if(msec<0){
		av_log(NULL, AV_LOG_ERROR, "%s: this->duration not set yet\n",__FUNCTION__);
		msec = 0;
	}
	//av_log(NULL, AV_LOG_DEBUG, "%s: into this->duration=%ld, msec=%d\n",__FUNCTION__,this->duration,msec);
	return msec;
}

int MediaPlayerSoft::is_looping()
{
	//av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);
...
int MediaPlayerSoft::is_buffering()
{
	//av_log(NULL, AV_LOG_DEBUG, "%s: into\n",__FUNCTION__);
...
int MediaPlayerSoft::setVolume(float left, float right)
{
...
int MediaPlayerSoft::config(const char* cfg, int val)
{
...
void  MediaPlayerSoft::notify(int msg, int ext1, int ext2, void *obj)
{    
	switch (msg) {    
